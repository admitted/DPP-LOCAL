...
<body style="background:#F5F3F0">
	...
	<form name="Analog_rainfall" action="Analog_rainfall.do" method="post" enctype="multipart/form-data">
		...
		<div style='position:absolute;width:60px;height:23px;right:120px;top:0px;background-color:#FFFFFF;overflow:auto;'>
  ...
  if(pPW != 'none')
  {
  	...
  	else
  	{
  		if(pType == "0")
			{
				pX = pY = 20;
				var pIcon  = '../skin/images/Water_3_start_New.gif';
			}else if(pType == "2")
			{
				pX = pY = 20;
				var pIcon  = '../skin/images/Water_3_end_New.gif';
			}else
			{
	  		pX = pY = 20;
	  		var pIcon  = '../skin/images/Water_3_New.gif';
	  	}
  	}
  }
  else
	    ...
	    if(reqRealGX.status == 200)
	    {
	      ...
	      if(null != Resp && Resp.length >= 4 && Resp.substring(0,4) == '0000')
	      {
	      	...
	      	for(var i=0; i<list.length && list[i].length>0; i++)
	      	{
	      		...
	      		if('00000' != document.getElementById("Func_Type_Map_Id").value)
						{
							var ftmi = "YJ" + sublist[0].substring(2,5);
