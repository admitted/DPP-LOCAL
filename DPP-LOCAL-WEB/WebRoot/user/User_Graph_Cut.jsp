...
<%
  ...
  CurrStatus currStatus = (CurrStatus)session.getAttribute("CurrStatus_" + Sid);
	...
	ArrayList    User_Graph_Cut_GX   = (ArrayList)session.getAttribute("User_Graph_Cut_GX_" + Sid);
	ArrayList    User_Graph_Cut_GJ   = (ArrayList)session.getAttribute("User_Graph_Cut_GJ_" + Sid);
...
<form name="User_Graph_Cut" action="User_Graph_Cut.do" method="post" target="mFrame" enctype="multipart/form-data">
  <div id="container" style="min-width:400px;min-height:320px"></div>
</form>
</body>
<script LANGUAGE="javascript">
 ...
 alert(description); 
	...
	var noWaterFlag = '<%=currStatus.getFunc_Id()%>';
	...
	var Data_Area_B = new Array();	//管线上无水区域		白色
...
<%
	if(null != User_Graph_Cut_GJ)
	{
		Iterator iter = User_Graph_Cut_GJ.iterator();
		while(iter.hasNext())
		{
	...
	%>
			...
			gjObj.Curr_Data 	= "<%=gjBean.getCurr_Data()%>";
			gjArray.push(gjObj);
	...
	if(null != User_Graph_Cut_GX)
	{
		Iterator it = User_Graph_Cut_GX.iterator();
		while(it.hasNext())
		{
...
%>		
	...
	{		
		...
		Data_Bar_A.push([xAx, parseFloat(tmpGJ.Top_Height), parseFloat(tmpGJ.Base_Height)]);
		if(1 != noWaterFlag)	//有水位图
		{
			...
			if(parseFloat(tmpGJ.Curr_Data) > 0)
			{
		  	Data_Bar_C.push([xAx, parseFloat(tmpGJ.Base_Height), parseFloat(tmpGJ.Base_Height) + parseFloat(tmpGJ.Curr_Data)]);
		  }
		  var gxTop_Height = parseFloat(gxBase_Height) + parseFloat(tmpGX.Diameter);
		  var gjWater_Lev  = parseFloat(tmpGJ.Base_Height) + parseFloat(tmpGJ.Curr_Data);
		  var gxWater_Lev = 0.0;
		  ...
		  //解决倒坡水位不正确问题 -- 2017.01.20.cj
		  var gxTop_Lev = 0.0;
		  if(gjWater_Lev > gxTop_Height)
		  {
		  	gxTop_Lev = gjWater_Lev + 0.05;
		  }
		  else
		  {
		  	gxTop_Lev = gxTop_Height;
		  }
		  Data_Area_B.push([xAx, parseFloat(gxTop_Height), parseFloat(gxTop_Lev)]);
		}
