  ...
  for(var i=0; i<tempArray.length; i++)
  {
	...
	if(confirm("确定提交?"))
	{
		...
		if(m_Edit)
		{
			...
			var url = 'Admin_User_RoleOP.do?Cmd=11&Sid=<%=Sid%>&Id='+role[0].id+'&CName='+role[0].name+'&Point='+point+'&currtime='+new Date();
			m_Edit.open("get", url);
			m_Edit.send(null);
			role[0].value = point;
		}
...
function doAdd(pId, pCName)
{
  ...
  if(m_Add)
  {
     ...
     var url = 'Admin_User_RoleOP.do?Cmd=10&Sid=<%=Sid%>&Id='+pId+'&CName='+pCName+'&currtime='+new Date();
     m_Add.open('get', url);
     m_Add.send(null);
  }
  else
...
function doDel(pId)
{
  ...
  if(m_Del)
  {
     ...
     var url = 'Admin_User_RoleOP.do?Cmd=12&Sid=<%=Sid%>&Id='+pId+'&currtime='+new Date();
     m_Del.open('get', url);
     m_Del.send(null);
  }
  else
